...
class TavilySearchTool(ToolBase):
    ...
    async def execute(
        self, query: str, results_count: int = 3, max_retries: int = 2
    ) -> ToolResponse:
        """
        ...
        while retry_count <= max_retries:
            try:
                logger.debug(
                    f"Making API request to Tavily (attempt {retry_count + 1})"
                )

                if has_tavily_package:
                    tavily_client = TavilyClient(api_key=api_key)
                    response_data = tavily_client.search(
                        query, max_results=results_count
                    )
                else:
                    headers = {
                    ...
                    payload = {"query": query, "max_results": results_count}
                ...
                if "results" in response_data:
                    ...
                    logger.success(
                        f"Found {len(results)} results from Tavily",
                ...
                if retry_count <= max_retries:
                    logger.warning(
                        f"Error in search attempt {retry_count}: {str(e)}. Retrying..."
                    )
                    await asyncio.sleep(2)  # Wait before retrying
                else:
                    logger.error(
                        f"All {max_retries+1} search attempts failed: {str(e)}"
                    )
                    return ToolResponse(
                        data={
                            "query": query,
