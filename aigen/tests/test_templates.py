...
def test_template_rendering():
    ...
    assert "class TestAgentAgent:" in code, "Generated code doesn't contain expected class definition"
    
    # Test YAML generation
    success, yaml_config = generator.generate_yaml_config(config)
    assert success, "Failed to generate YAML config"
    assert "agent_type: test_agent" in yaml_config, "Generated YAML doesn't contain expected content"
