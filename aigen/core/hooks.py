...
class ContentWorkflowHooks(RunHooks):
        ...
        try:
            ...
            if self.debug_mode:
                ...
                if hasattr(context_wrapper.context, 'get_state_summary'):
                    ...
                    logger.debug(f"Context state before '{agent.name}' starts:", context_state)
                
                if agent.name == "Content Strategist" and hasattr(context_wrapper.context, 'has_research'):
                    if not context_wrapper.context.has_research():
                        logger.warning(f"No research data found before '{agent.name}' starts!")
                
                if agent.name == "Content Writer" and hasattr(context_wrapper.context, 'has_outline'):
                    if not context_wrapper.context.has_outline():
                        logger.warning(f"No outline data found before '{agent.name}' starts!")
                
                if agent.name == "Content Editor" and hasattr(context_wrapper.context, 'has_draft'):
                    if not context_wrapper.context.has_draft():
                        logger.warning(f"No draft content found before '{agent.name}' starts!")
                
        except Exception as e:
            logger.error(f"Error in on_agent_start: {str(e)}")
    
    async def on_agent_end(self, context_wrapper: RunContextWrapper, agent, output) -> None:
        ...
        try:
            ...
            if isinstance(output, str):
                output_text = output
                logger.debug(f"Using direct string output from agent: {len(output_text)} chars")
            elif hasattr(agent, 'messages') and agent.messages and len(agent.messages) > 0:
                last_message = agent.messages[-1]
                if hasattr(last_message, 'content'):
                    output_text = last_message.content
                    logger.debug(f"Using content from agent's last message: {len(output_text)} chars")
                else:
                    output_text = str(last_message)
                    logger.debug(f"Using string representation of agent's last message: {len(output_text)} chars")
            else:
                output_text = str(output)
                logger.debug(f"Using stringified output parameter: {len(output_text)} chars")
            ...
            if output_text:
                ...
                if hasattr(context_wrapper, 'context'):
                    ...
                    storage_success = self._store_agent_output(ctx, agent.name, output_text)
                    
                    if storage_success:
                        self.successful_completions.add(agent.name)
                        logger.success(f"Agent '{agent.name}' finished successfully", 
                            outputs_length=output_length,
                            output_sample=output_text[:100] + "..." if len(output_text) > 100 else output_text
                        )
                    else:
                        logger.warning(f"Agent '{agent.name}' finished but output storage failed")
                    
                    if self.debug_mode and hasattr(ctx, 'get_state_summary'):
                        context_state = ctx.get_state_summary()
                        logger.debug(f"Context state after '{agent.name}' completes:", context_state)
            else:
    ...
    def _store_agent_output(self, ctx, agent_name: str, output: str) -> bool:
        ...
        try:
            ...
            if agent_name == "Research Specialist" and hasattr(ctx, 'set_research_data'):
                return ctx.set_research_data(output)
            elif agent_name == "Content Strategist" and hasattr(ctx, 'set_outline_data'):
        ...
        try:
            ...
            if hasattr(context, 'context'):
                ...
                if hasattr(from_agent, 'messages') and from_agent.messages:
                    ...
                    logger.debug(f"Handoff message from {from_agent.name}: {output_text[:100]}...")
                    
                    if from_agent.name not in self.successful_completions:
                        storage_success = self._store_agent_output(ctx, from_agent.name, output_text)
                        if storage_success:
    ...
    async def on_tool_start(self, context_wrapper: RunContextWrapper, agent, tool) -> None:
    ...
    async def on_tool_end(self, context_wrapper: RunContextWrapper, agent, tool, result: str) -> None:
        ...
        try:
            if tool and agent and hasattr(agent, 'name'):
                ...
                if self.debug_mode:
                    result_snippet = str(result)[:100] + "..." if len(str(result)) > 100 else str(result)
    ...
    async def on_run_end(self, context_wrapper: RunContextWrapper) -> None:
        ...
        try:
            if hasattr(context_wrapper, 'context'):
                ...
                if self.debug_mode and hasattr(ctx, 'get_state_summary'):
                    context_state = ctx.get_state_summary()
                    logger.debug(f"Final context state after running agents:", context_state)
        ...
        try:
            ...
            if has_final:
                ...
                if outputs:
                    ...
                    logger.info(f"Workflow completed with output from {last_agent}", 
                        characters=last_len,
                        time_taken=f"{elapsed:.2f}s"
                    )
                else:
                    logger.error(f"Workflow completed but no content was generated!", 
                        time_taken=f"{elapsed:.2f}s",
