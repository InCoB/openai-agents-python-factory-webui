...
class AgentRegistrationService:
    ...
    def register_agent_type(
        self, config: AgentConfiguration, code: str
    ) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            logger.warning(
                "Code-based registration failed for %s, falling back to config-based: %s",
                config.agent_type,
                message,
            )
    ...
    def _register_from_code(
        ...
        try:
            try:
                ...
                return (
                    False,
                    f"Syntax error in generated code at line {line_no}: {str(se)}",
                )
            ...
            if not agent_class:
                return (
                    False,
                    f"Could not find class {class_name} in the generated module",
                )
            ...
            try:
                ...
                register_agent_factory(
                    config.agent_type,
                    lambda agent_id=None, **kwargs: getattr(module, class_name)(
                        agent_id=agent_id or config.agent_type, **kwargs
                    ),
                ...
                return (
                    True,
                    f"Successfully registered {config.name} ({config.agent_type})",
                )
    ...
    def _register_from_config(self, config: AgentConfiguration) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            def factory_function(
                ...
                for tool_name in config.tools:
                    try:
                        ...
                        logger.warning("Failed to add tool %s: %s", tool_name, str(e))
            ...
            register_agent_factory(
                config.agent_type, factory_function, {"description": config.name}
            )

            return (
                True,
                f"Successfully registered {config.name} ({config.agent_type}) from config",
            )
    ...
    def unregister_agent(self, agent_type: str) -> Tuple[bool, str]:
        ...
        try:
            try:
                ...
                elif hasattr(agent_registry, "get_factories"):
                    # Use a getter method instead of accessing protected members
                    factories = agent_registry.get_factories()
                    if agent_type in factories:
                        del factories[agent_type]

                    if hasattr(agent_registry, "get_metadata"):
                        metadata = agent_registry.get_metadata(name=agent_type)
                        if agent_type in metadata:
                            del metadata[agent_type]
                else:
            ...
            try:
                ...
                logger.warning("Error cleaning up cached files: %s", str(e))
