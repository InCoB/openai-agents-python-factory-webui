...
def register_workflow_factory(
    name: str, factory: Callable, metadata: Optional[Dict[str, Any]] = None
) -> None:
    """
...
def create_workflow(
    workflow_spec: Union[str, List[str], Dict[str, Any], Path], **kwargs
):
    """
        ...
        if isinstance(workflow_spec, str):
            ...
            if workflow_spec == "research_only":
                ...
                return DeterministicWorkflow(
                    agents=[create_agent("research")], name="research_only"
                )
            ...
            if workflow_spec in workflow_registry.list():
                logger.debug(
                    f"Creating workflow from registered factory: {workflow_spec}"
                )
                ...
                if callable(factory):
                    try:
                        ...
                        logger.warning(
                            f"Workflow factory error for {workflow_spec}: {str(e)}"
                        )
                        # Fall back to other approaches below
                else:
                    logger.warning(
                        f"Factory for {workflow_spec} is not callable, trying other approaches"
                    )
            ...
            raise ValueError(
                f"Unsupported workflow specification type: {type(workflow_spec)}"
            )
...
def create_workflow_by_type(workflow_type: str, **kwargs):
    ...
    if workflow_type == "deterministic":
        ...
        try:
            ...
            class_name = (
                "".join(word.capitalize() for word in workflow_type.split("_"))
                + "Workflow"
            )
    ...
    factories = [
        ...
        (
            ...
            lambda agents=None, name="deterministic", **kwargs: (
                __import__(
                    "aigen.workflows.deterministic", fromlist=["DeterministicWorkflow"]
                ).DeterministicWorkflow(agents=agents or [], name=name, **kwargs)
            ),
        ...
        (
            ...
            lambda agents=None, name="handoff", **kwargs: (
                __import__(
                    "aigen.workflows.handoff", fromlist=["HandoffWorkflow"]
                ).HandoffWorkflow(agents=agents or [], name=name, **kwargs)
            ),
        ...
        (
            ...
            lambda **kwargs: (
                __import__(
                    ...
                    agents=[create_agent("research")], name="research_only", **kwargs
                )
            ),
    ...
    for workflow_type, factory_func, description in factories:
        try:
            register_workflow_factory(
                workflow_type, factory_func, {"description": description}
            )
