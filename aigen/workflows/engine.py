...
class WorkflowEngine:
    ...
    async def execute(self, 
        ...
        try:
            ...
            if hasattr(workflow, "execute_with_trace"):
                ...
                result = await workflow.execute_with_trace(context, input_text, workflow_id)
            elif hasattr(workflow, "execute_deterministic"):
                result = await workflow.execute_deterministic(context, input_text, trace_id=workflow_id)
            elif hasattr(workflow, "execute_handoff"):
                result = await workflow.execute_handoff(context, input_text, trace_id=workflow_id)
    ...
    async def execute_agent(self, 
                          agent: AgentBase, 
                          context: Context, 
                          input_text: Optional[str] = None) -> AgentResponse:
        """
        ...
        active_agents = self.active_workflows.get(getattr(context, '_workflow_id', 'unknown'), {}).get('agents_executed', [])
        ...
        while retry_count <= self.max_retries:
            try:
                logger.info(f"Executing agent {agent.agent_id} (attempt {retry_count + 1})")
                ...
                if response.success:
                    ...
                    logger.warning(f"Agent {agent.agent_id} reported failure: {error_msg}")
                    ...
                    if retry_count <= self.max_retries:
                        logger.info(f"Retrying agent {agent.agent_id} in {self.retry_delay}s")
