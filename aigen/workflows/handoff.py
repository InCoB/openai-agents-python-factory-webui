...
class HandoffWorkflow:
    ...
    async def execute_handoff(self, context: Context, input_text: Optional[str] = None, trace_id: Optional[str] = None) -> str:
        """
        ...
        with trace(f"Handoff: {self.name}", trace_id=trace_id):
            ...
            try:
                ...
                if final_output:
                    logger.info(f"Handoff workflow completed with output from final agent {final_agent_id}")
                ...
                for agent in reversed(self.agents):
                    ...
                    if output:
                        logger.info(f"Handoff workflow completed with partial output from agent {agent.agent_id}")
                        return output
                ...
                for agent in reversed(self.agents):
                    output = context.get_output(agent.agent_id)
                    if output:
                        logger.info(f"Returning partial output from agent {agent.agent_id} after error")
                        return f"[PARTIAL RESULT] {output}"
                
                raise WorkflowError(f"Handoff workflow failed: {str(e)}", workflow_id=self.name)