    ...
    async def execute_deterministic(
        ...
        with trace(f"Workflow: {self.name}", trace_id=trace_id):
            ...
            for i, agent in enumerate(self.agents):
                ...
                response = await self.engine.execute_agent(
                    agent, context, current_input
                )
                ...
                if not response.success:
                    ...
                    if i > 0:
                        logger.warning(
                            f"Agent {agent_id} failed but we have partial results"
                        )
                ...
                if len(self.agents) == 1:
                    logger.info(
                        f"Single-agent workflow completed successfully with {agent_id}"
                    )
            ...
            if not final_output:
                logger.warning(
                    "No output from final agent, attempting to get best available content"
                )
                return self._get_best_content(context)
