...
class AgentManagerUI:
    ...
    async def delete_agent(self, agent_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            config_success, config_message = self.persistence.delete_agent_config(
                agent_type
            )
            ...
            if not config_success:
                return {
                    "status": "error",
                    "message": f"❌ Failed to delete agent configuration: {config_message}",
                }
            ...
            try:
                ...
                if os.path.exists(pycache_dir):
                    ...
                    for cached_file in glob.glob(
                        os.path.join(pycache_dir, f"{agent_type}.*")
                    ):
            ...
            try:
                ...
                if not success and "not found in registry" in message:
                    ...
                    return {
                        "status": "success",
                        "message": f"✅ Agent '{agent_type}' deleted successfully",
                    }
                ...
                logger.warning(
                    f"Error unregistering agent (config still deleted): {str(e)}"
                )
                return {
                    "status": "success",
                    "message": f"✅ Agent '{agent_type}' deleted successfully",
        ...
        with gr.Tab("Agent Management") as tab:
            ...
            def on_delete_and_refresh(agent_type: str):
                ...
                return (
                    result,
                    agents_list,
                    gr.Dropdown(choices=agent_choices, value=None),
                )

            refresh_btn.click(on_refresh, outputs=[agent_table, agent_dropdown])
