...
class GradioInterface:
    ...
    def _get_workflow_choices(self) -> List[str]:
        ...
        filtered_workflows = [name for name in self.available_workflows.keys() 
    ...
    def _get_workflow_agents(self, workflow_name: str) -> List[str]:
        ...
        # Hardcoded workflow agent configurations
        workflow_agents = {
            "content_generation": ["research", "strategy", "writer", "editor"],
            "research_only": ["research"],
            # "content_analysis": ["research", "editor"]  # Removed - was causing issues
        }
    ...
    async def run_workflow(self, 
        ...
        try:
            ...
            if workflow_name == "custom":
                ...
                logger.info(f"Creating custom workflow with agents: {', '.join(custom_agents)}")
            else:
            ...
            def update_progress(current_step, total_steps, message):
                ...
                percent = 0.1 + (0.8 * (current_step / total_steps)) if total_steps > 0 else 0.5
            ...
            trace_url = f"https://platform.openai.com/traces/{workflow_id}" if workflow_id else ""
    ...
    def build_ui(self) -> gr.Blocks:
        ...
        with gr.Blocks(
            ...
            with gr.Tabs() as tabs:
                ...
                with gr.TabItem("Use Predefined Workflow"):
                    ...
                    workflow_name = gr.Dropdown(
                        ...
                        value="content_generation" if "content_generation" in self._get_workflow_choices() else None,
                        label="Workflow Type",
                    ...
                    execute_predefined_btn = gr.Button("Execute Workflow", variant="primary")
                    
                    # Output section
                    gr.Markdown("## Output")
                    predefined_output_text = gr.Textbox(
                        label="Result",
                        lines=20
                    )
                ...
                with gr.TabItem("Create Custom Workflow"):
                    ...
                    gr.Markdown("*Select agents in the order they should execute. The order matters!*")
                    ...
                    execute_custom_btn = gr.Button("Execute Custom Workflow", variant="primary")
                ...
                with gr.TabItem("Workflow Management"):
                    ...
                    workflows_table = gr.DataFrame(
                        value=[[name, info["description"]] for name, info in self.available_workflows.items()],
                    ...
                    agents_table = gr.DataFrame(
                        value=[[name, info["description"]] for name, info in self.available_agents.items()],
                        headers=["Name", "Description"],
                        label="Agents"
                    )
            ...
            execute_predefined_btn.click(
                ...
                inputs=[workflow_name, gr.State([]), predefined_input_text, max_turns_predefined],
