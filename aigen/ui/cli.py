...
def get_selection(prompt: str, options: List[str], default: Optional[str] = None) -> str:
    """
...
async def interactive_mode() -> None:
    ...
    workflow_selection = get_selection(
        ...
        workflow_choices,
        default="content_generation" if "content_generation" in available_workflows else None
    )
    ...
    if workflow_selection == "custom":
        ...
        while True:
            ...
            selection = get_selection(f"Select agent #{len(custom_agents) + 1}", agent_options)
...
async def main_cli() -> int:
    ...
    execute_parser.add_argument("--max-retries", type=int, default=3, help="Maximum retries for failed agent executions")
    execute_parser.add_argument("--retry-delay", type=int, default=5, help="Delay between retries in seconds")
    ...
    web_parser.add_argument("--share", action="store_true", help="Create a publicly shareable link")
